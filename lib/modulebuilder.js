...
ModuleBuilder.prototype.compile = function(opt_callback) {
  ...
  this._parser.parse(function(err, rootModule) {
    ...
    self.runCompiler(rootModule, function(err, data) {
      if (err) return callback(err);

      if (self._mapTempFile) {
        self._mapTempFile.readFile('utf8', function(err, mapData) {
          self._mapTempFile.unlink();
          self._mapTempFile = null;

          if (err) return callback(null, data);

          /** @type {!Array<string>} */
          var moduleNames = self._modulesInfo.map(function(jsModule) {
          ...
          moduleNames.forEach(function(moduleName, i) {
            if (mapData && mapData[i]) {
              fs.writeFile(
                self.sourceMapPath + moduleName.split(':')[0] + '.map',
                mapData[i], 'utf8', onComplete);
            } else {
              onComplete();
            }
          });
...
ModuleBuilder.prototype.getArgs = function(rootModule, callback) {
  var f = function(err, javaArgs, compilerArgs) {
    ...
    if (this.sourceMapPath) {
      ...
      try {
        this._mapTempFile = new tmp.File();
        compilerArgs.push(
          '--create_source_map "' + this._mapTempFile.path + '"');
      } catch (e) { }
    }
