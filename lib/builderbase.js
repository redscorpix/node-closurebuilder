...
class BuilderBase {
  ...
  getArgs(rootModule) {
    return Promise.resolve([
      ...rootModule.getDeps(true).map(jsSource => `--js "${jsSource.path}"`),
  ...
  runCompiler(rootModule, cb) {
    return Promise.all([
      isJavaCompiler(this.compilerPath) ?
          this._getJavaArgs() : Promise.resolve([this.compilerPath]),
      this.getArgs(rootModule),
    ]).then(data => {
      const [fileArgs, compilerArgs] = data;

      return {compilerArgs, fileArgs};
    }).then(data => this._runCompilerCompile(data));
  }

  /**
   * @param {{compilerArgs:!Array<string>,fileArgs:!Array<string>}} data
   * @return {!Promise<string>}
   * @private
   */
  _runCompilerCompile(data) {
    const {compilerArgs, fileArgs} = data;
 ...
 * @return {boolean}
 */
const isJavaCompiler = compilerPath =>
    '.jar' === require('path').extname(compilerPath);
