...
/**
 * @param {JsModule} rootModule
 * @param {function(Error,string)} cb First argument is error or
 *    null. Second argument is the compiled source, as a string, or empty
...
BuilderBase.prototype.runCompiler = function(rootModule, cb) {
  ...
  var compile = function(err, javaArgs, compilerArgs) {
    ...
    tempFile.writeFile(compilerArgs.join(' '), function(err) {
      ...
      var command = javaArgs.concat(
        ['--flagfile "' + tempFile.path + '"']).join(' ');
      var commandLog = '';
