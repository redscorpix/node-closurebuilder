 ...
 * @param {function(Error,Array.<string>)} callback The first argument
 *    is error or null. The second argument is version.
 * @protected
 */
...
BuilderBase.prototype.runCompiler = function(rootModule, callback) {
  var compile = function(err, args) {
    if (err) {
      ...
      if (BuilderBase.LogLevel.SHORT == this._logLevel) {
        var jsFlag = false;
        var commandArgs = [];
        ...
        var commandLog = commandArgs.join(' ');
      } else if (BuilderBase.LogLevel.LONG == this._logLevel) {
