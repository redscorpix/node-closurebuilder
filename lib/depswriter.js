 ...
 * @constructor
 */
var DepsWriter = exports.DepsWriter = function(opt_jsFiles, opt_baseDir,
    opt_prefix) {
...
DepsWriter.prototype._getSourcesMap = function(preData, callback) {
  ...
  async.eachSeries(preData, function(item, callback) {
    ...
    utils.findSourcesByJsFiles(item.jsFiles, function(err, jsSources) {
      if (!err) {
        jsSources.forEach(function(jsSource) {
          if (item.path) {
            pathToSource[item.path] = jsSource;
          } else {
            var sourceFile = jsSource.getPath();
            var depPath = item.prefix + path.relative(baseDir, sourceFile);
 ...
 *    null. Second argument is content.
 */
exports.build = function(options, opt_callback) {
